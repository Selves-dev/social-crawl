...
# Builds the application for different environments
...
if [[ "$ENVIRONMENT" == "prod" ]]; then
    # For production builds, temporarily use production environment
    if [[ -f ".env.secrets" ]]; then
        echo -e "${BLUE}🔧 Using production environment for build...${NC}"
        cp .env.secrets .env.prod.tmp
        # Add base .env values that aren't secrets
        cat .env >> .env.prod.tmp
        NODE_ENV=production DOTENV_CONFIG_PATH=.env.prod.tmp pnpm run build
        rm -f .env.prod.tmp
    else
        echo -e "${YELLOW}⚠️ No .env.secrets found, building with current environment${NC}"
...
EOF
    ...
    # Create production .env file from secrets and base config
    if [[ -f ".env.secrets" ]]; then
        echo -e "${BLUE}📋 Creating production .env file...${NC}"
        # Copy production secrets
        cp .env.secrets "$BUILD_DIR/.env"
        # Append non-secret base configuration
        echo "" >> "$BUILD_DIR/.env"
        echo "# Base configuration from .env" >> "$BUILD_DIR/.env"
        grep -v "^#\|^$" .env >> "$BUILD_DIR/.env"
    else
        echo -e "${YELLOW}⚠️ No .env.secrets found, using template${NC}"
        cp .env "$BUILD_DIR/.env"
    fi
    
    # Also create environment template for Azure Key Vault deployment
