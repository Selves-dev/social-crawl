...
export function buildAnalysisPrompt(blobJson: any, workflow: any): string {
  ...
  return [
    ...
    '',
    '--- ADDITIONAL CONTEXT FIELDS (REQUIRED FOR ALL VENUES) ---',
    'Beyond the core venue data, you MUST also gather the following contextual information:',
    '',
    '**Price Tier Classification:**',
    '- Search for pricing data from booking sites, reviews, menus, or official sources',
    '- Classify using: "$" (Budget), "$$" (Mid-range), "$$$" (Upscale), "$$$$" (Luxury)',
    '- For hotels: Base on average nightly rates in the local market context',
    '- For restaurants: Base on average meal cost per person',
    '- For experiences: Base on typical ticket/entry prices',
    '',
    '**Unique Selling Points:**',
    '- Identify 3-5 distinctive, factual characteristics that differentiate this venue',
    '- Focus on verifiable claims: awards, historical significance, unique features, architectural details, location advantages',
    '- Examples: "Michelin 2-star restaurant", "Grade I listed building from 1720", "Only hotel with direct cathedral views", "Award-winning rooftop bar"',
    '- Avoid generic marketing language like "warm hospitality" or "great service"',
    '- If fewer than 3 can be verified, provide what you can find',
    '',
    '**Aggregate Reviews:**',
    '- Search for review scores across major platforms (Google, TripAdvisor, Booking.com for hotels, OpenTable for restaurants)',
    '- Calculate or identify an aggregate score out of 5 (e.g., 4.6)',
    '- Count total number of reviews across all sources',
    '- Document source as: "Aggregated across [list platforms found]"',
    '- If no reviews found, set all fields to null',
    '',
    '**Neighborhood & Points of Interest:**',
    '- Identify the specific neighborhood/district/quarter name where the venue is located (e.g., "Covent Garden", "Marais", "Cathedral Quarter")',
    '- Find 3-5 notable points of interest within realistic walking distance (typically under 15 minutes)',
    '- For each POI include:',
    '  * name: Official name of the landmark/site',
    '  * walk_time_minutes: Approximate walking time (integer)',
    '  * category: One of: "Historic Site", "Museum", "Transport Hub", "Park", "Shopping", "Dining", "Entertainment", "Religious Site"',
    '- Only include POIs where walking distance can be verified or reasonably estimated',
    '- Prioritize major landmarks and transport hubs over generic amenities',
    '',
    '--- DATA SCHEMAS & JSON STRUCTURES ---',
    '',
    // ...existing code for schemas and restaurant part...
