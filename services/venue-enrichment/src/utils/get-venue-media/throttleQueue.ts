...
export class GetVenueMediaThrottleQueue {
  ...
  constructor() {
    this.queueName = process.env["asb-get-venue-media-queue"] || 'get-venue-media';
    this.maxConcurrentJobs = parseInt(process.env["get-venue-media-max-concurrent-jobs"] || '2', 10);
  }
  ...
  async initialize(): Promise<void> {
    ...
    logger.debug('Get venue media throttle queue initialized', {
      service: 'get-venue-media',
  ...
  async sendJob(job: PostOfficeMessage): Promise<void> {
    ...
    const messageId = `${job.util || 'get-venue-media'}_${job.type}_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
    ...
    logger.debug(`Get venue media job queued: ${messageId}`, {
      service: 'get-venue-media',
      jobId: messageId,
      jobType: job.type
    });
  ...
  subscribe(handler: (message: PostOfficeMessage) => Promise<void>): void {
    if (!this.receiver) {
      throw new Error('GetVenueMediaThrottleQueue not initialized. Call initialize() first.');
    }
    ...
    this.receiver.subscribe({
      processMessage: async (msg: ServiceBusReceivedMessage) => {
        try {
          ...
          if (!security.validateMessageSecurity(msg.applicationProperties)) {
            logger.error('Get venue media job failed security validation', new Error('Invalid security token'), {
              service: 'get-venue-media',
      ...
      processError: async (args) => {
        logger.error('GetVenueMediaThrottleQueue processing error', args.error, {
          service: 'get-venue-media',
  ...
  async stop(): Promise<void> {
    ...
    logger.info('Get venue media throttle queue stopped', { service: 'get-venue-media' });
  }
}

// Export singleton instance
export const getVenueMediaQueue = new GetVenueMediaThrottleQueue();
