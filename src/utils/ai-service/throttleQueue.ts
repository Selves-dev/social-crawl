...
export class AIServiceThrottleQueue {
  ...
  async sendJob(job: PostOfficeMessage): Promise<void> {
    ...
    const messageId = `${job.util || 'ai'}_${job.type}_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
    ...
    logger.info(`AI job queued: ${messageId}`, {
      service: 'ai-service',
      jobId: messageId,
      jobType: job.type
    });
  ...
  subscribe(handler: (message: PostOfficeMessage) => Promise<void>): void {
    if (!this.receiver) {
      throw new Error('AIServiceThrottleQueue not initialized. Call initialize() first.');
    }
    ...
    this.receiver.subscribe({
      ...
      processError: async (args) => {
        logger.error('AIServiceThrottleQueue processing error', args.error, {
          service: 'ai-service',
